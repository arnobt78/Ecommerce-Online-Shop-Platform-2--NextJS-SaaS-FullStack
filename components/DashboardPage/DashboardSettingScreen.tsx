...
function DashboardSettingScreen() {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center px-4 py-8 bg-[#F7F8FD] rounded-2xl relative">
      ...
      {showCancelPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
          ...
          >
            ...
            <div className="text-center text-lg font-medium text-[#222] mb-2">
              {t("dashboard.subscriptionCanceled")}
            </div>
            <Button
              className="mt-4 px-6 py-2 rounded-lg bg-[#20AE5C] text-white font-semibold"
              onClick={() => {
      ...
      {/* ...existing code... */}
      <h2 className="text-4xl font-semibold text-[#1B223C] mb-2">
        {t("dashboard.setting")}
      </h2>
      <div className="text-base text-[#1B223C] mb-4 flex items-center gap-2">
        ...
        {isActive ? (
          <span className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#1B223C] font-medium">
              {t("dashboard.active")}
            </span>
          </span>
        ) : (
        ...
        <div className="space-y-4 mb-8 text-base sm:text-lg">
          <div className="flex items-center space-x-4">
            ...
            <span className="text-md sm:text-base text-gray-800">
              {t("dashboard.knowledgeHours")}
            </span>
          </div>
          <div className="flex items-center space-x-4">
            <Check className="h-4 sm:h-5 w-4 sm:w-5 text-green-600 flex-shrink-0" />
            <span className="text-md sm:text-base text-gray-800">
              {t("dashboard.knowRisks")}
            </span>
          </div>
          <div className="flex items-center space-x-4">
            <Check className="h-4 sm:h-5 w-4 sm:w-5 text-green-600 flex-shrink-0" />
            <span className="text-md sm:text-base text-gray-800">
              {t("dashboard.avoidAddiction")}
            </span>
          </div>
          <div className="flex items-center space-x-4">
            <Check className="h-4 sm:h-5 w-4 sm:w-5 text-green-600 flex-shrink-0" />
            <span className="text-md sm:text-base text-gray-800">
              {t("dashboard.beHealthy")}
            </span>
          </div>
        </div>
        ...
        {isActive ? (
          <Button
            ...
            onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => {
              e.currentTarget.style.backgroundColor = "rgba(248, 0, 43, 0.1)";
            }}
