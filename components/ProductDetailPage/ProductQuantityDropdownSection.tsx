 ...
 */
const ProductQuantityDropdownPrice: React.FC<
  ProductQuantityDropdownPriceProps
> = ({ salePrice, originalPrice, value = 1 }) => {
 ...
 */
const ProductQuantityDropdownList: React.FC<
  ProductQuantityDropdownListProps
> = ({ quantityOptions, value, onChange, listRef }) => (
  <div
    ref={listRef}
    className="absolute left-1/2 -translate-x-1/2 z-50 mt-1 w-[110px] max-h-[303px] bg-white border border-black/15 rounded-xl shadow-2xl overflow-y-auto mx-auto"
    style={{
    ...
    {quantityOptions.map((q, i) => (
      <button
        ...
        style={{
          ...
          lineHeight: "27px",
          borderBottom:
            i !== quantityOptions.length - 1 ? "1px solid #EFEFEF" : undefined,
        }}
        onClick={() => {
          onChange(q);
        }}
...
export const ProductQuantityDropdownSection: React.FC<
  ProductQuantityDropdownSectionProps
> = ({ value, onChange, min = 1, max = 10, salePrice, originalPrice }) => {
    ...
    >
      <div ref={dropdownRef} className="relative">
        <button
          ...
          aria-expanded={open}
          className="flex flex-row items-center w-[75px] h-[45px] sm:h-[51px] bg-white border border-black/15 rounded-xl px-4 py-2 shadow focus:outline-none focus:ring-2 focus:ring-[#8EF7FB] cursor-pointer"
          style={{
