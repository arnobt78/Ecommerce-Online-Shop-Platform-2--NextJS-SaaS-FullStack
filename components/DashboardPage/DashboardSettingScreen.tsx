...
function DashboardSettingScreen() {
  const [showCancelPopup, setShowCancelPopup] = React.useState(false);
  const [isActive, setIsActive] = React.useState(true);

  return (
    <div className="w-full h-full flex flex-col items-center px-4 py-8 bg-[#F7F8FD] rounded-2xl relative">
      {/* Cancel Subscription Popup */}
      {showCancelPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
          <div className="bg-white rounded-2xl shadow-lg p-8 flex flex-col items-center" style={{ minWidth: 320, minHeight: 120 }}>
            <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className="mb-4">
              <circle cx="32" cy="32" r="32" fill="#EAFBF3"/>
              <path d="M20 34L28.5 42L44 26.5" stroke="#20AE5C" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <div className="text-center text-lg font-medium text-[#222] mb-2">Subscription on snuzz PRO is canceled</div>
            <Button className="mt-4 px-6 py-2 rounded-lg bg-[#20AE5C] text-white font-semibold" onClick={() => { setShowCancelPopup(false); setIsActive(false); }}>Close</Button>
          </div>
        </div>
      )}
      {/* ...existing code... */}
      <h2 className="text-4xl font-semibold text-[#1B223C] mb-2">Setting</h2>
      <div className="text-base text-[#1B223C] mb-4 flex items-center gap-2">
        <span>Subscription status:</span>
        {isActive ? (
          <span className="flex items-center gap-2">
            <span className="inline-block w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
            <span className="text-sm text-[#1B223C] font-medium">Active</span>
          </span>
        ) : (
          <span className="flex items-center gap-2">
            <span className="inline-block w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
            <span className="text-sm text-[#1B223C] font-medium">Deactive</span>
          </span>
        )}
      </div>
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-md p-8 flex flex-col items-center border border-[#E7EBFF]">
        <svg viewBox="0 0 212 40" fill="none" xmlns="http://www.w3.org/2000/svg" className="mb-4 mx-auto w-[190px] h-[36px] sm:w-[210px] sm:h-[38px] ">
        ...
        <div className="text-[#A1A1A1] text-base mb-4 text-center">Unleash the Power of Knowladge with snuzz PRO</div>
        <hr className="w-full border-t border-[#E7EBFF] my-4" />
        ...
        {isActive ? (
          <Button
            ...
            style={{ borderColor: "#F8002B", color: "#F8002B" }}
            onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.backgroundColor = "rgba(248, 0, 43, 0.1)"; }}
          ...
          <Button
            ...
            onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.backgroundColor = "rgba(34, 211, 238, 0.1)"; }}
            onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.backgroundColor = "transparent"; }}
            disabled
          >
            Try for free
...
export default DashboardSettingScreen;
