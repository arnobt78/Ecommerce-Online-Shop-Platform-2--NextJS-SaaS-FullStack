  ...
  return (
    <div className="mt-6">
      ...
      <div className="border rounded-xl overflow-hidden">
        ...
        {selected === "card" && (
          <form onSubmit={handleSubmit(onSubmit)} className="px-4 pt-4 pb-2 bg-gray-50">
            <div className="mb-3 relative">
              <input {...register("cardNumber")} placeholder="Card number" className={`w-full border rounded px-3 py-2 pr-10 ${errors.cardNumber ? 'border-red-500' : 'border-gray-300'}`} maxLength={19} />
              <CreditCard className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
              {errors.cardNumber && <p className="text-xs text-red-600 mt-1">{errors.cardNumber.message}</p>}
            </div>
            <div className="flex gap-2 mb-3">
              <div className="flex-1 relative">
                <input {...register("expiry")} placeholder="MM/YY" className={`w-full border rounded px-3 py-2 pr-8 ${errors.expiry ? 'border-red-500' : 'border-gray-300'}`} maxLength={5} />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs">MM/YY</span>
                {errors.expiry && <p className="text-xs text-red-600 mt-1">{errors.expiry.message}</p>}
              </div>
              <div className="flex-1 relative">
                <input {...register("cvc")} placeholder="CVC" className={`w-full border rounded px-3 py-2 pr-8 ${errors.cvc ? 'border-red-500' : 'border-gray-300'}`} maxLength={4} />
            ...
            <div className="mb-3">
              <input {...register("name")} placeholder="Name on card" className={`w-full border rounded px-3 py-2 ${errors.name ? 'border-red-500' : 'border-gray-300'}`} />
              {errors.name && <p className="text-xs text-red-600 mt-1">{errors.name.message}</p>}
            </div>
            <label className="flex items-center mb-3 cursor-pointer">
              <input type="checkbox" {...register("useShipping")} className="accent-black mr-2" defaultChecked />
              <span className="text-sm">Use shipping address as billing address</span>
            </label>
            <button type="submit" className="w-full bg-black text-white py-2 rounded font-bold flex items-center justify-center gap-2">
              <Lock className="w-4 h-4 mr-1" /> Pay Securely
            </button>
          </form>
        )}
        {/* Klarna Option */}
        <div className={`border-t border-b ${selected === "klarna" ? "bg-pink-50" : "bg-white"}`}>
          <div className="flex items-center px-4 py-3">
            <input type="radio" id="klarna" name="payment-method" checked={selected === "klarna"} onChange={() => setSelected("klarna")}
          ...
          {selected === "klarna" && (
            <div className="flex flex-col items-center px-4 pb-6 pt-2 text-center animate-fade-in">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="-252.3 356.1 163 80.9" className="w-36 h-36"><path fill="none" stroke="currentColor" stroke-miterlimit="10" stroke-width="1" d="M-108.9 404.1v30c0 1.1-.9 2-2 2H-231c-1.1 0-2-.9-2-2v-75c0-1.1.9-2 2-2h120.1c1.1 0 2 .9 2 2v37m-124.1-29h124.1"></path><circle cx="-227.8" cy="361.9" r="1.8" fill="currentColor"></circle><circle cx="-222.2" cy="361.9" r="1.8" fill="currentColor"></circle><circle cx="-216.6" cy="361.9" r="1.8" fill="currentColor"></circle><path fill="none" stroke="currentColor" stroke-miterlimit="10" stroke-width="2" d="M-128.7 400.1H-92m-3.6-4.1 4 4.1-4 4.1"></path></svg>
              <p className="text-sm text-gray-700 mb-1">After you review your order, you will be redirected to Klarna to securely complete your purchase.</p>
            </div>
          )}
        </div>
        {/* Online Bank Transfer Option */}
        <div className={`border-b ${selected === "bank" ? "bg-blue-50" : "bg-white"}`}>
          <div className="flex items-center px-4 py-3">
            <input type="radio" id="bank" name="payment-method" checked={selected === "bank"} onChange={() => setSelected("bank")}
              className="accent-blue-500 mr-2" />
            <label htmlFor="bank" className={`${selected === "bank" ? "font-semibold" : "font-normal"} flex-1 cursor-pointer`}>Online Bank Transfer</label>
            <img src={cardIcons.find(icon => icon.alt === "Bank Transfer")?.src} alt="Bank Transfer" className="h-8 w-10 object-cover bg-white rounded shadow border border-gray-200" />
          </div>
          {selected === "bank" && (
            <div className="px-6 pb-6 pt-2 animate-fade-in">
              <div className="mb-3">
                <span className="block font-medium mb-2">Select payment method:</span>
        ...
        <div className={`${selected === "wire" ? "bg-yellow-50" : "bg-white"}`}> 
          <div className="flex items-center px-4 py-3">
            <input type="radio" id="wire" name="payment-method" checked={selected === "wire"} onChange={() => setSelected("wire")}
              className="accent-blue-500 mr-2" />
            <label htmlFor="wire" className={`${selected === "wire" ? "font-semibold" : "font-normal"} flex-1 cursor-pointer`}>Wire Transfer</label>
            <img src={cardIcons.find(icon => icon.alt === "iDEAL")?.src} alt="iDEAL" className="h-8 w-10 object-cover bg-white rounded shadow border border-gray-200" />
          </div>
          {selected === "wire" && (
            <div className="px-6 pb-6 pt-2 animate-fade-in">
              <p className="text-sm text-gray-700 mb-1">After you complete your order, you will receive payment instructions for the wire transfer.</p>
              <p className="text-xs text-gray-600">→ Important: Your order will be shipped after your payment is received.</p>
            </div>
          )}
        </div>
      </div>
