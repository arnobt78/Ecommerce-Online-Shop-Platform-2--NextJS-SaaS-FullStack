...
export const CategoryFilterMenuBar: React.FC<CategoryFilterMenuBarProps> = ({
  ...
  // Filter data with translations
  const filterData = [
    {
      title: t("products.filter.brands"),
      options: getUnique(products.map((p) => p.brand)),
    },
    {
      title: t("products.filter.flavors"),
      options: getUnique(products.map((p) => p.flavor)),
    },
    {
      title: t("products.filter.strength"),
      options: getUnique(products.map((p) => p.strength)),
    },
    {
      title: t("products.filter.sort"),
      options: [
  ...
  const strengthIcon = (
    ...
    >
      <path
        d="M23.4375 24.3068H17.1875V3.99426H23.4375V24.3068ZM18.75 22.7443H21.875V5.55676H18.75V22.7443ZM15.625 24.3068H9.375V10.2443H15.625V24.3068ZM7.8125 24.3068H1.5625V14.9318H7.8125V24.3068Z"
        fill="black"
      />
    </svg>
  );
          ...
          <FilterDropDown
            ...
            disabledOptions={(() => {
              ...
              if (selectedSort === "Price Low to High")
                return ["Price High to Low", "Products A-Z", "Products Z-A"];
              if (selectedSort === "Price High to Low")
                return ["Price Low to High", "Products A-Z", "Products Z-A"];
              if (selectedSort === "Products A-Z")
                return [
                  "Products Z-A",
                  "Price Low to High",
                  "Price High to Low",
                ];
              if (selectedSort === "Products Z-A")
                return [
                  "Products A-Z",
                  "Price Low to High",
                  "Price High to Low",
